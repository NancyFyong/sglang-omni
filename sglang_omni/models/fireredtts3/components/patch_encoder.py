...
# Vendored from https://github.com/FireRedTeam/FireRedTTS3 (Apache-2.0).
# Kept verbatim except for rewritten import paths; excluded from autoflake
# in .pre-commit-config.yaml so re-exports survive.
import torch

from sglang_omni.models.fireredtts3.components.modules import (
    DiTBlock,
    FinalLayer,
    RotaryEmbedding,
)
...
class PatchEncoder(torch.nn.Module):
    ...
    def forward(self, inputs_embeds: torch.Tensor):
        ...
        """
        assert (
            inputs_embeds.shape[1] % self.patch_size == 0
        ), "inputs_embeds.shape={} patch_size={}".format(
            inputs_embeds.shape, self.patch_size
        )
        ...
        cls_tok = self.cls_tok.expand(
            hidden_states.shape[0], -1, -1
        )  # (b*t//patch_size, 1, c)
        hidden_states = torch.cat(
            [cls_tok, hidden_states], dim=1
        )  # (b*t//patch_size, 1+patch_size, c)
        # NOTE full attention
